...
function Analytics() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useAnalytics(companyId, eventMap[dataType]);
  ...
  const computedStats = useMemo(() => {
    ...
    if (dataType === 'pages') {
      ...
      const totalCompletions = Object.values(analyticsResponse.data).reduce(
        (acc: number, items: any) =>
          acc + items.reduce((sum: number, d: any) => sum + d.secondary, 0),
        0,
      );
      return [
        {
      ...
      return [
        ...
        },
        {
          ...
          value: (totalPrimary / 90).toFixed(1),
          change: '',
          icon: TrendingUp,
          color: 'text-green-500',
        },
          ...
          >
            ...
            <div className='relative z-10 space-y-3'>
              <div className='flex items-center justify-between'>
                <h3 className='text-sm font-semibold text-foreground opacity-80'>
                  Data Source
                </h3>
        ...
        <div className='w-full'>
          {isLoading ? (
            ...
            <div className='w-full h-[400px] flex flex-col items-center justify-center bg-card rounded-2xl border border-border/80 text-muted-foreground gap-4'>
              ...
              <p className='text-xs'>
                Try selecting a different data source or check back later.
              </p>
            </div>
          ) : (
